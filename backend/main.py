...
async def update_task(task_id: str, updates: TaskUpdate):
    ...
    new_desc = updates.description if updates.description is not None else current["description"]
    new_status = updates.status if updates.status is not None else current["status"]
    new_spec = updates.specialization if updates.specialization is not None else current["specialization"]
    new_priority = updates.priority if updates.priority is not None else current["priority"]
    new_blocked = json.dumps(updates.blocked_by) if updates.blocked_by is not None else current["blocked_by"]
    new_metadata = json.dumps(updates.metadata) if updates.metadata is not None else current["metadata"]
...
async def delete_task(task_id: str):
    ...
    cursor = await db.execute("SELECT id, blocked_by FROM tasks WHERE blocked_by LIKE ?", (f'%{task_id}%',))
...
@app.get("/api/stats/tasks", response_model=TaskStatsResponse)
async def get_task_stats(
    ...
    # Query 3: Group by specialization
    spec_cursor = await db.execute(
        f"SELECT specialization, COUNT(*) as count FROM tasks {where} GROUP BY specialization",
        params,
    )
    by_specialization = {
        row["specialization"]: row["count"] for row in await spec_cursor.fetchall()
    }
