...
- **Statuses (columns)**: backlog → queued → in_progress → needs_review → needs_human → done
...
- **Dependencies**: tasks have `blocked_by` list with lazy validation — references to non-existent task IDs are accepted and treated as non-blocking. `is_blocked` is true only when a blocker exists AND isn't `done`
